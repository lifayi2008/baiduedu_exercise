...
//选择排序处理程序
function selectOrderHandler(event) {
    if(boxListData.length < 2) {
        return;
    }
    for(var i = 1; i < boxListData.length; i++) {
        var x = boxListData[i];
        for(var j = i - 1; j >= 0; j--) {
            if(x < boxListData[j]) {
                boxListData[j+1] = boxListData[j];
                continue;
            } else {
                break;
            }
...
function renderBoxList() {
    ...
    for(var i = 0; i < boxListData.length; i++) {
        ...
        box.style.height = boxListData[i] * 2 + "px";
...
function action() {
    ...
    eBtnLeftIn.onclick = function() {
        ...
        if(inputValue && 10 <= inputValue && inputValue <= 100) {
    ...
    var eSelectOrder = document.getElementById("selectOrder");
    eSelectOrder.onclick = selectOrderHandler;
}
